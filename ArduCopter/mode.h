...
class ModeMP : public Mode {

public:
    // inherit constructor
    using Mode::Mode;
    Number mode_number() const override { return Number::MP; }
    ...
    SubMode submode() const { return mp_mode; }
...
protected:
    ...
    const char *name4() const override { return "MP"; }
...
private:
    ...
    // controls which controller is run (pos or vel):
    SubMode mp_mode = SubMode::VelAccel;
    bool send_notification;     // used to send one time notification to ground station
    bool takeoff_complete;      // true once takeoff has completed (used to trigger retracting of landing gear)
